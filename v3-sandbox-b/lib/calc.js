...
// 动态计算同业数：先看 spot 是否有显式 sameCount_xxx，否则按距 IFC 距离 + 业态密度系数推
export function getSameCount(spot, categoryId){
  const cached = spot['sameCount_' + categoryId];
  if (cached != null) return cached;
  const cat = CATEGORIES[categoryId];
  const dCenter = Math.hypot(spot.x, spot.z);
  // 基础密度（咖啡=1.0 时的同业数公式）× 业态密度乘子
  const base = Math.max(8, Math.round(40 - dCenter/12));
  const mul = (cat && cat.densityMul) || 0.5;
  return Math.max(2, Math.round(base * mul));
}

export function calcK(spot, categoryId){
  const cat = CATEGORIES[categoryId];
  const Nsame = getSameCount(spot, categoryId);
